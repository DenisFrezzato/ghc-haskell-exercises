...
-- We don't know anything about @a@, so @f@ must be a function that returns
-- a constant value.

-- | c. How do we "get back" knowledge about what's in the list? Can we?
-- No, it's a lossy transformation.
...
-- foldr f o (CanFold x) = foldr f o x
...
unpackEqPair eq (EqPair x y) = eq x y
...
-- pairs?
unpackEqPairs :: (forall a. Eq a => a -> a -> b) -> EqPair -> EqPair -> (b, b)
unpackEqPairs eq xs ys = (unpackEqPair eq xs, unpackEqPair eq ys)
...
unpackNestedX ::
  (forall subinput suboutput. Nested input output subinput suboutput -> b) ->
...
-- We don't need to keep track of subinput and suboutput types and we gain flexibility.
...
unwrapMB :: (forall a. MysteryBox a -> b) -> MysteryBox a -> Maybe b
unwrapMB _ EmptyBox = Nothing
unwrapMB f (IntBox _ m) = Just $ f m
unwrapMB f (StringBox _ m) = Just $ f m
unwrapMB f (BoolBox _ m) = Just $ f m

-- | b. Why do we need a 'Maybe'? What can we still not know?
-- We don't have an @a@ in @EmptyBox@.

-- | c. Write a function that uses 'unwrap' to print the name of the next
-- layer's constructor.
showNextLayer :: MysteryBox a -> String
showNextLayer box =
  fromMaybe "No inner box" $
    unwrapMB showConstructor box
  where
    showConstructor :: MysteryBox a -> String
...
filter' :: (a -> Bool) -> Vector n a -> (forall m. Vector m a -> b) -> b
filter' _ VNil f = f VNil
filter' p (VCons x xs) f
  | p x = filter' p xs (f . VCons x)
