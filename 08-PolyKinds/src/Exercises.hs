...
newtype Tagged (name :: Symbol) (a :: Type) = Tagged {runTagged :: a}
...
-- generalise this?
newtype Tagged' (name :: k) (a :: Type) = Tagged' {runTagged' :: a}

-- | b. Can we generalise 'Type'? If so, how? If not, why not?
-- No, because 'runTagged' must be a type of kind 'Type'.
...
-- A tag can be narrowed down to a finite number of tags.
...
filterVector :: (a -> Bool) -> Vector a n -> Sigma (Vector a)
filterVector _ VNil = Sigma SZ VNil
filterVector p (VCons h t)
  | p h = cons h (filterVector p t)
  | otherwise = filterVector p t
  where
    cons h (Sigma n t) = Sigma (SS n) (VCons h t)
...
serverLog (x : xs) = case x of
  Sigma SServer (ServerComm c) -> c : serverLog xs
...
serverLog' :: [Communication l] -> [ServerData]
serverLog' [] = []
serverLog' (x : xs) = case x of
  ServerComm c -> c : serverLog' xs
  ClientComm _ -> serverLog' xs
